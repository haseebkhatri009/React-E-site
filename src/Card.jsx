...
export default function card(Props) {
  ...
  return (
    <div className="h-[350px] border border-transparent hover:border-red-200 w-[270px] mb-6">
      <div className="h-[250px] border-red-400 overflow-hidden relative group">
        {/* <div className="h-[26px] w-[60px] border-black rounded-md mt-3 ml-3 bg-red-500 text-white px-2">
            {Props.discount}
        </div> */}
        <div className=" bg-white overflow-hidden">
          <img
            className="  w-full h-[200px] object-contain  transform transition-transform duration-300 ease-in-out group-hover:scale-110"
            src={Props.img}
            alt=""
          />
        </div>
        <div
          ...
          className="h-[35px] border border-black bg-black text-white text-center cursor-pointer hover:text-red-500 absolute top-[212px] w-[270px] py-1"
        >
          View Details
        </div>
      </div>
      <p className=" mt-4 font-semibold overflow-hidden truncate ">
        {Props.desc}
      </p>
      <p className="mt-[6px]">
        ...
        <span className="ml-3">{Props.discount}</span>
      </p>
      {/* <ReactStars
        count={5}
        value={Props.rating}
